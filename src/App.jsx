  ...
  return (
    <div className="page">
      <div className="sh">
        ...
        <p className="sh-sub">World Cup 2026 · Kicks off June 11 · Threshold: <strong style={{color:"#ffd700"}}>XI (11)</strong></p>
      </div>
...
const XIBuilder = ({ auth, group, type, onSave, onBack, onToast }) => {
  ...
  const posOptions = ["ALL","GK","CB","LB","RB","CDM","CM","CAM","LW","RW","ST","CF"];
  const filtered = PLAYERS.filter(p => {
    if (posFilter!=="ALL" && p.pos!==posFilter) return false;
    if (search && !p.name.toLowerCase().includes(search.toLowerCase()) && !p.nation.toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  });

  const toggle = (id) => setSelectedIds(prev => prev.includes(id) ? prev.filter(x=>x!==id) : prev.length<11 ? [...prev,id] : prev);
  const selectedPlayers = PLAYERS.filter(p => selectedIds.includes(p.id));
  const total = selectedPlayers.reduce((s,p) => s+(type==="goals"?p.goals:p.cards), 0);
  const {score,bust} = calcScore(total);
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const payload = { user_id: auth.user.id, group_id: group.id, type, xi: selectedIds, bonus_answers: bonusAnswers };
      if (existing) {
      ...
      onToast("Failed to save entry — try again", true);
    }
    setSaving(false);
  };
  ...
  if (step==="bonus") return (
    <div className="page-narrow">
      ...
      <div className="sh">
        ...
        <p className="sh-sub">Up to +5 bonus points</p>
      </div>
      {bonusQs.map(q => (
        <div className="card" style={{marginBottom:10}} key={q.id}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:15}}>{q.label}</div>
            <div style={{background:"rgba(255,215,0,.1)",border:"1px solid rgba(255,215,0,.25)",borderRadius:4,padding:"2px 9px",fontSize:12,color:"#ffd700",fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700}}>+{q.points} pts</div>
          </div>
          <input className="form-input" placeholder="Player or country name..." value={bonusAnswers[q.id]||""} onChange={e => setBonusAnswers({...bonusAnswers,[q.id]:e.target.value})} />
        </div>
      ))}
  ...
  if (step==="review") return (
    <div className="page-narrow">
      ...
      <div className="sh">
        <div className="sh-eyebrow">{group.name}</div>
      ...
      <div className="card" style={{marginBottom:14}}>
        <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:11,letterSpacing:2,color:"#3a5a3a",marginBottom:12}}>YOUR STARTING XI</div>
        <div style={{display:"flex",flexDirection:"column",gap:5}}>
          {selectedPlayers.map(p => (
            <div key={p.id} style={{display:"flex",alignItems:"center",gap:9,background:"#122612",borderRadius:6,padding:"7px 10px"}}>
              <span>{p.flag}</span>
              <span className={`pos-badge ${p.pos}`}>{p.pos}</span>
              <span style={{flex:1,fontFamily:"'Barlow Condensed',sans-serif",fontWeight:600,fontSize:13}}>{p.name}</span>
              <span style={{fontSize:11,color:"#7a9a7a"}}>{type==="goals"?`⚽ ${p.goals}`:`🟨 ${p.cards}`}</span>
            </div>
          ))}
        </div>
        <hr />
      ...
      <button className="btn btn-gold" style={{width:"100%",padding:16,fontSize:15,gap:10}} onClick={save} disabled={saving}>
  ...
  return (
    <div className="page">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        ...
        <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:20,letterSpacing:1}}>{type==="goals"?"⚽ Goals XI":"🟨 Cards XI"} · {group.name}</div>
        <button className="btn btn-primary btn-sm" disabled={selectedIds.length!==11} onClick={() => setStep("bonus")}>Next: Bonuses →</button>
      </div>
      <div className="xi-builder-layout">
        <div>
          <input className="form-input" style={{marginBottom:10,padding:"8px 12px",fontSize:13}} placeholder="Search players or nations..." value={search} onChange={e => setSearch(e.target.value)} />
          <div className="filter-bar">
            {posOptions.map(p => <button key={p} className={`filter-chip ${posFilter===p?"active":""}`} onClick={() => setPosFilter(p)}>{p}</button>)}
          </div>
          <div className="players-grid">
            {filtered.map(p => <PlayerCard key={p.id} player={p} selected={selectedIds.includes(p.id)} disabled={!selectedIds.includes(p.id)&&selectedIds.length>=11} onClick={() => toggle(p.id)} type={type} />)}
          </div>
        </div>
        <div className="xi-panel">
          <div className="card">
            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:11,letterSpacing:2,color:"#3a5a3a",marginBottom:12}}>SELECTED: {selectedIds.length} / 11</div>
            {selectedIds.length===0
              ? <div style={{textAlign:"center",padding:"24px 0",color:"#3a5a3a",fontSize:13}}>Pick 11 players</div>
              : <div className="xi-list">{selectedPlayers.map(p => (
                  <div className="xi-item" key={p.id}>
                    <span>{p.flag}</span>
                    <span className="xi-item-name">{p.name}</span>
                    <span style={{fontSize:10,color:"#3a5a3a"}}>{type==="goals"?p.goals:p.cards}</span>
                    <button className="xi-remove" onClick={() => toggle(p.id)}>×</button>
                  </div>
                ))}</div>
            }
            {selectedIds.length>0 && (<><hr /><ProgressBar current={total} /><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:6}}><span style={{fontSize:11,color:"#7a9a7a"}}>Score now:</span><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:24,color:bust?"#ff3d3d":score>=9.5?"#ffd700":"#00c853"}}>{bust?"💥":`${score}pts`}</div></div></>)}
          </div>
        </div>
      </div>
    </div>
