        ...
        {Object.keys(FORMATIONS).map(f => {
          ...
          return (
            <div
              ...
              style={{
                background:"#0c210c",border:"1px solid #1a3a1a",borderRadius:12,
            ...
            >
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:28,letterSpacing:2,color:"#ffd700"}}>{f}</div>
                <div style={{fontSize:12,color:"#7a9a7a"}}>{fSlots.filter(s=>s.group==="DEF").length} DEF · {fSlots.filter(s=>s.group==="MID").length} MID · {fSlots.filter(s=>s.group==="FWD").length} FWD</div>
              </div>
              {/* Mini pitch preview */}
              <div style={{background:"#071a07",borderRadius:8,padding:"10px 8px",border:"1px solid #1a3a1a"}}>
                {rows.map(row => (
                  <div key={row} style={{display:"flex",justifyContent:"center",gap:8,marginBottom:row<rows[rows.length-1]?8:0}}>
                    {fSlots.filter(s=>s.row===row).map(s => (
                      <div key={s.id} style={{
                        ...
                        color:"#7a9a7a",letterSpacing:0.5,
                      }}>{s.label}</div>
                    ))}
                  </div>
                ))}
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,alignItems:"start"}}>
          ...
          <div>
            <div style={{
              ...
              {/* Slots by row — reversed so GK at bottom */}
  ...
  return (
    <div className="page-narrow">
      ...
      <div className="card" style={{marginBottom:14}}>
        ...
        {[...new Set(formationSlots.map(s=>s.row))].reverse().map(row => (
